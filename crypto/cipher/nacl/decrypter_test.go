...
func TestDecrypter_Decrypt_sr25519(t *testing.T) {
	assert := assert.New(t)
	type fields struct {
		privateKey crypto.PrivateKey
	}
...
func Test_validatePrivateKeyType_sr25519(t *testing.T) {
	assert := assert.New(t)
	type args struct {
		pk crypto.PrivateKey
	}
	tests := []struct {
	...
	}{
		{
			...
			[]byte{48, 120, 50, 51, 98, 48, 54, 51, 97, 53, 56, 49, 102, 100, 56, 101, 53, 101, 56, 52, 55, 99, 52, 101, 50, 98, 57, 99, 52, 57, 52, 50, 52, 55, 50, 57, 56, 55, 57, 49, 53, 51, 48, 102, 53, 50, 57, 51, 98, 101, 51, 54, 57, 101, 56, 98, 102, 50, 51, 97, 52, 53, 100, 50, 98, 100},
			false,
		},
		{
			"err-secp256k1-sofia",
