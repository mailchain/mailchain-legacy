...
func TestPathAddress(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			got, err := PathAddress(tt.args.r, tt.args.protocol)
			if (err != nil) != tt.wantErr {
...
func TestPathProtocol(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			got, err := PathProtocol(tt.args.r)
			if (err != nil) != tt.wantErr {
				t.Errorf("PathProtocol() error = %v, wantErr %v", err, tt.wantErr)
				return
			}
			if got != tt.want {
				t.Errorf("PathProtocol() = %v, want %v", got, tt.want)
			}
		})
	}
}
