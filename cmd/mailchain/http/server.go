...
func CreateRouter(s *settings.Root, mailbox stores.State, cmd *cobra.Command) (http.Handler, error) {
	...
	api.HandleFunc("/messages", handlers.GetMessages(config.receivers, config.mailboxStateStore, config.cache, config.keystore, deriveKeyOptions)).Methods("GET") //nolint:lll
