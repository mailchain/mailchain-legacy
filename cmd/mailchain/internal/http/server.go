...
func CreateRouter(s *settings.Root, mailbox stores.State, cmd *cobra.Command) (http.Handler, error) {
	...
	api.HandleFunc("/balance", handlers.GetBalance(config.keystore)).Methods("GET")

	api.HandleFunc("/envelope", handlers.GetEnvelope(config.sentStore)).Methods("GET")

	api.HandleFunc("/messages", handlers.GetMessages(config.mailboxStateStore, config.cache, config.keystore, deriveKeyOptions)).Methods("GET") //nolint:lll
	...
	api.HandleFunc("/messages/fetch", handlers.FetchMessages(config.mailboxStateStore, config.receivers, config.keystore)).Methods("POST")
